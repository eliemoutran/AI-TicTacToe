...
class TicTacToe:
    ...
    # TODO minimax
    def minimax(self, depth, isMax): # if isMax I'm on the max node else on the min node
        # Base case
    ...
    def bestMoveAB(self, player): 
        ...
        for i in range(3):
            for j in range(3):
                if self.board[i][j] == '-':
                    self.set_board(i,j,player)
                    score = self.AlphaBetaPruning(0, False, -float('inf'), float('inf'))
                    # Undo your move
                    self.set_board(i,j,'-')
                    if (score > bestScore):
                        bestScore = score
